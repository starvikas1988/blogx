<x-app-layout>
  ...
  <form method="POST" action="{{ route('posts.update', $post) }}" class="space-y-4 bg-white p-6 rounded-xl shadow">
    @csrf @method('PUT')
    <div>
      <label class="block text-sm font-medium mb-1">Title</label>
      <input name="title" class="w-full border rounded p-2" value="{{ old('title', $post->title) }}" required>
